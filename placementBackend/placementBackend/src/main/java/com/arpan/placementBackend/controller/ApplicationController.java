...
public class ApplicationController {
    ...
    public ResponseEntity<ApiResponse<ApplicationResponse>> applyForJob(
            @AuthenticationPrincipal User currentUser,
            @PathVariable Long jobId) {
        ApplicationResponse response = applicationService.applyForJob(currentUser.getId(), jobId);
        return ResponseEntity.ok(ApiResponse.success("Application submitted successfully.", response));
    }
    ...
    /**
     * GET /api/v1/student/applications/{applicationId}
     * Only the student who owns the application may fetch it via this URL.
     */
    @GetMapping("/api/v1/student/applications/{applicationId}")
    public ResponseEntity<ApiResponse<ApplicationResponse>> getApplicationByIdAsStudent(
            @AuthenticationPrincipal User currentUser,
            @PathVariable Long applicationId) {
        JobApplication app = applicationService.getApplicationOrThrow(applicationId);
        if (!app.getStudent().getId().equals(currentUser.getId())) {
            throw new UnauthorizedException("You can only view your own applications.");
        }
        return ResponseEntity.ok(ApiResponse.success(
                "Application fetched.", applicationService.mapToResponse(app)));
    }
    ...
    public ResponseEntity<ApiResponse<ApplicationResponse>> previewMatch(
            @AuthenticationPrincipal User currentUser,
            @PathVariable Long jobId) {
        ApplicationResponse preview = applicationService.previewMatch(currentUser.getId(), jobId);
        return ResponseEntity.ok(ApiResponse.success("Match preview generated.", preview));
    }
    ...
    /**
     * NEW — GET /api/v1/recruiter/applications/{applicationId}
     * The recruiter can fetch any application that belongs to one of their jobs.
     * This was the missing piece: without it, recruiters could not open the
     * applicant detail page (the previous code routed through /student/** which
     * SecurityConfig restricts to ROLE_STUDENT, causing 403).
     */
    @GetMapping("/api/v1/recruiter/applications/{applicationId}")
    public ResponseEntity<ApiResponse<ApplicationResponse>> getApplicationByIdAsRecruiter(
            @AuthenticationPrincipal User currentUser,
            @PathVariable Long applicationId) {
        JobApplication app = applicationService.getApplicationOrThrow(applicationId);
        if (!app.getJob().getRecruiter().getId().equals(currentUser.getId())) {
            throw new UnauthorizedException(
                    "You can only view applications for your own job listings.");
        }
        return ResponseEntity.ok(ApiResponse.success(
                "Application fetched.", applicationService.mapToResponse(app)));
    }
    ...
    @PostMapping("/api/v1/recruiter/applications/{applicationId}/recompute-match")
    public ResponseEntity<ApiResponse<ApplicationResponse>> recomputeMatch(
            @AuthenticationPrincipal User currentUser,
            @PathVariable Long applicationId) {
        ApplicationResponse updated = applicationService.recomputeAiMatch(
                currentUser.getId(), applicationId);
        return ResponseEntity.ok(ApiResponse.success("AI match recomputed.", updated));
    }
}