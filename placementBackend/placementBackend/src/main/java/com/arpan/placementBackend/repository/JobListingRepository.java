...
public interface JobListingRepository extends JpaRepository<JobListing, Long> {
    ...
    @Query("SELECT j FROM JobListing j JOIN FETCH j.recruiter WHERE j.recruiter = :recruiter")
    List<JobListing> findByRecruiter(@Param("recruiter") User recruiter);

    @Query("SELECT j FROM JobListing j JOIN FETCH j.recruiter WHERE j.active = true")
    List<JobListing> findByActiveTrue();

    @Query("SELECT j FROM JobListing j JOIN FETCH j.recruiter WHERE j.id = :id")
    Optional<JobListing> findByIdWithRecruiter(@Param("id") Long id);

    @Query("SELECT j FROM JobListing j JOIN FETCH j.recruiter " +
            "WHERE j.active = true " +
